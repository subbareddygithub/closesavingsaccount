...
@Service
public class ServicingOrderRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(ServicingOrderRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.servicingorder.hosturl}")
	private String url;
	
	@Value("${spring.api.servicingorder.context-path}")
	private String contextPath;

	@Value("${spring.api.servicingorder.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  ServicingOrderRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ ServicingOrderRestProcessor.class.getName());
